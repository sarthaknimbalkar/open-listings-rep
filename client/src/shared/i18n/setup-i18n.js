...
export const setupI18n = async () => {
    ...
    i18next
        ...
        .then(function () {
            if (cookies['locale']) {
                i18next.changeLanguage(cookies['locale']).then(() => {
                    ...
                    if (cookies['locale'] === 'ar') {
                        ...
                        document.getElementsByTagName('header')[0].children[0].className = "container-fluid d-flex align-items-center"
                    }
                    const langOptions = document.getElementsByTagName('option')
                    const opt = [...langOptions].find(
                        (opt) => opt.value === cookies['locale'] || opt.value.startsWith(cookies['locale'])
                    )
