...
export const setupInputTags = async () => {
    ...
    const promises = dataURLs.map((url) => fetch(url, { credentials: 'include' }).then((y) => y.json()))
    Promise.all(promises)
        .then((arr) => {
            // state.tags['ar'] = state.tags['ar'] ? state.tags['ar'] : getTags(arr[0].tags)
            state.tags['en-US'] = state.tags['en-US'] ? state.tags['en-US'] : getTags(arr[0].tags)
            state.tags['fr'] = state.tags['fr'] ? state.tags['fr'] : getTags(arr[1].tags)
            tagifyPage()
