...
export function moveableMarker(map, marker, coordinates) {
    ...
    if (map['name'] === 'geoSearchMap' || map['name'] === 'addListingMap') {
    ...
    marker.on('mouseup', () => {
        ...
        if (isMarkerInsidePolygon(marker, coordinates)) {
            const center = marker.getLatLng ? marker.getLatLng() : marker.getBounds().getCenter()
            if (map['name'] === 'addListingMap') {
                const where = polygons.findIndex((coo) => isMarkerInsidePolygon(marker, coo))
            ...
            if (lastValid.length) marker.setLatLng(lastValid)
        }
    })
