...
import { addListingMap } from './create-maps/create-add-listing-map.js'
...
export const setupMaps = () => {
    ...
    // Listing page map
    if (LIS.id('add-listing-map') && ['markets', 'events'].indexOf(__section__) > -1) {
        map = addListingMap({ lat, lng, layerFactory, zoom })
        maps.push(map)
    }
    // Geo search map
