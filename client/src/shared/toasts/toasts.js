...
const __suggestions__ = window['__suggestions__']
...
export const runToasts = async () => {
    const notyfInfo = new Notyf({
        types: [
            { type: 'info', background: 'blue', icon: false, duration: 9000 },
            { type: 'suggestion', backgroundColor: 'linear-gradient(45deg, rgb(239, 253, 33), rgb(255, 0, 0))', icon: false, duration: 9000 }
        ]
    })
    ...
    __suggestions__.forEach((suggestion) => {
        notyfInfo.open({ type: 'suggestion', message: suggestion })
    })
    ...
    const sometimes = Math.random() < 0.2
